...
/*
* print wait message to screen when waitng download
* @param
* @return void
*/
void printWatingMsg() {
...
int showListSelectUser(char* listUser, char* username, char* fileName) {
	...
	int i;
	printf("\n---------- List User ------------\n");
	printf(" Username\t\t\tFile\t\t\tSize(bytes)\n");
	for (i = 0; *(list + i); i++)
    {
    ...
    int option;
    while(1) {
	    printf("\nPlease select owner to download file (Press 0 to cancel): ");
	    scanf("%s", choose);
		while(getchar() != '\n');
		option = atoi(choose);
		if((option >= 0) && (option <= i)) {
			...
			printf("Syntax error. Please Enter Valid Options!\n");
		}
	}
...
void handleSearchFile() {
	...
	if(fptr != NULL) {
		while(1) {
			printf("\nYou have already had a file with the same name!\n -- Are you want to continue search? y/n: ");
			scanf("%c", &choose);
			while(getchar() != '\n');
...
*/
void manual() {
	printf("\n-- To search and download file:\n\t PRESS 1 and type exact file name\n");
	printf("-- To view your list file:\n\t PRESS 2\n");
...
int main(int argc, char const *argv[])
{
	...
	if(argc!=3){
		printf("Error!\nPlease enter two parameter as IP-Address and Port number!\n");
		exit(0);
	}
